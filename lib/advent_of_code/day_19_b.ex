defmodule AdventOfCode.Day19B do
  ...
  defp run({grid, {x, y}, direction}, result \\ [], steps \\ 0) do
    case Map.get(grid, {x, y}, " ") do
      " " -> 
        {steps, Enum.reverse result}
      "+" ->
        {{new_x, new_y}, new_direction} = turn grid, {x, y}, direction
        run {grid, {new_x, new_y}, new_direction}, result, steps + 1
      path when path in ["-", "|"] ->
        run {grid, move({x, y}, direction), direction}, result, steps + 1
      letter -> 
        run {grid, move({x, y}, direction), direction}, [letter | result], steps + 1
    end
  end
