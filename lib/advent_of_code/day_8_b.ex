defmodule AdventOfCode.Day8B do

  def max_memory(input) do
    input
    |> Stream.reject(& &1 == "")
    |> Stream.map(&String.trim/1)
    |> Stream.map(&parse_command/1)
    |> Stream.scan(%{}, fn command, registers -> eval command, registers end)
    |> Enum.flat_map(& &1 |> Enum.map(fn {_, v} -> v end))
    |> Enum.max
  end
